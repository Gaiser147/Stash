...
class NavidromeCoverResolver @Inject constructor(
    ...
    suspend fun resolve(
        ...
        albumArtPath?.takeIf { it.isNotBlank() }?.let { return it }
        ArtUrlUpgrader.upgrade(albumArtUrl)?.takeIf { it.isNotBlank() }?.let { return it }

        if (lastFmCredentials.isConfigured && artist.isNotBlank() && title.isNotBlank()) {
            val key = "${artist.trim().lowercase()}\n${title.trim().lowercase()}"
            lastFmCache[key]?.takeIf { it.isNotBlank() }?.let { return it }
            runCatching {
                lastFmApiClient.getTrackInfo(artist, title).getOrNull()?.bestImageUrl
            }.getOrNull()
